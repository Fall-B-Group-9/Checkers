...
public class GameEngine {
	...
	/********************** Evaluation Function **********************************
	 * 
	 * For normal checkers; Eval=(weightOfChecker +
	 * PositionWeight*(position)^2)*numberOfCheckers + randomWeight;
	 * 
	 * For kings; Eval=(weightOfKing - EdgeWeight*numberOfEdges)*numberOfKings +
	 * randomWeight;
	 * 
	 * Weight of checkers Own normal checker = 100 Opposite normal checker =
	 * -100 Own king = 200 Opposite king = -200
	 * 
	 * For normal checkers there is a weight for position. The evaluted are less
	 * in edges for kings, so that they tend to be in the middle Small random
	 * weight between 0-10 was added to reduce the monotony of the game.
	 **********************************************************************************/
	...
	/********************
	 * MinMax with Alpha-Beta Pruning ************************ returns the best
	 * score evaluated MinMax algorithm with alpha beta pruning cut off the tree
	 * according to the difficulty level With maximum difficulty it searches
	 * up to 6 levels for the best possible solution With minimum difficulty it
	 * searches only up to two levels.
	 ****************************************************************************/
	...
	static int MinMax(int[][] board, int depth, int maxDepth, int[] move, int turn, int[] counter, int redBest,
			int yellowBest) {
		int score, bestScore;
		int[][] newBoard;
		...
		if (depth == maxDepth) {
			...
			for (int i = 0; i < movesList.size(); i++) {
				...
				CheckerMove.moveComputer(newBoard, (int[]) movesList.elementAt(i)); // returns new_board (changing start and end 
																					//coordinates and applying the move)
				int temp[] = new int[4];
